import random
import statistics

from gv_baseline.godscore import godscore, GVConfig


def _make_candidate(rng, n=6):
    # Candidate has order/entropy signals AND its own GVConfig (heritable)
    order = [rng.uniform(0.0, 5.0) for _ in range(n)]
    entropy = [rng.uniform(0.0, 5.0) for _ in range(n)]

    cfg = GVConfig(
        alpha=rng.uniform(0.5, 1.5),
        beta=rng.uniform(0.5, 1.5),
        gamma=rng.uniform(0.5, 2.0),
    )
    return (order, entropy, cfg)


def _mutate_cfg(rng, cfg, sigma=0.08):
    # Small gaussian mutation, keep within sane bounds
    def clamp(x, lo, hi):
        return max(lo, min(hi, x))

    return GVConfig(
        alpha=clamp(cfg.alpha + rng.gauss(0.0, sigma), 0.1, 5.0),
        beta=clamp(cfg.beta + rng.gauss(0.0, sigma), 0.1, 5.0),
        gamma=clamp(cfg.gamma + rng.gauss(0.0, sigma), 0.1, 10.0),
    )


def _mutate_signals(rng, xs, sigma=0.15):
    return [max(0.0, x + rng.gauss(0.0, sigma)) for x in xs]


def _mutate_candidate(rng, cand):
    order, entropy, cfg = cand
    return (
        _mutate_signals(rng, order),
        _mutate_signals(rng, entropy),
        _mutate_cfg(rng, cfg),
    )


def _score(cand):
    order, entropy, cfg = cand
    return godscore(order, entropy, cfg)


def test_parameter_evolution_improves_population_and_best():
    rng = random.Random(987654)

    pop_size = 350
    generations = 14
    survival_frac = 0.30  # top 30% survive

    population = [_make_candidate(rng) for _ in range(pop_size)]

    mean_scores = []
    best_scores = []
    best_cfgs = []

    for _ in range(generations):
        scored = sorted((( _score(c), c) for c in population), key=lambda t: t[0], reverse=True)
        scores = [s for s, _ in scored]

        mean_scores.append(statistics.mean(scores))
        best_scores.append(scores[0])
        best_cfgs.append(scored[0][1][2])  # cfg from best candidate

        survivors = [c for _, c in scored[: int(pop_size * survival_frac)]]

        # Reproduce + mutate
        new_population = []
        while len(new_population) < pop_size:
            parent = rng.choice(survivors)
            child = _mutate_candidate(rng, parent)
            new_population.append(child)

        population = new_population

    # Core claims:
    # 1) Mean score improves meaningfully
    assert mean_scores[-1] > mean_scores[0] + 0.06

    # 2) Best-of-generation improves as well
    assert best_scores[-1] > best_scores[0] + 0.03

    # 3) The best configuration changes (i.e., evolution is actually doing something)
    #    (Not asserting directionality yet—just non-stasis)
    distinct_cfgs = len({(round(c.alpha, 2), round(c.beta, 2), round(c.gamma, 2)) for c in best_cfgs})
    assert distinct_cfgs >= 4
